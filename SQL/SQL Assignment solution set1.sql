...
SELECT * FROM CITY WHERE COUNTRYCODE = 'USA' AND POPULATION > 90555;
...
SELECT City, LENGTH(City) AS Min_Length_City FROM Station ORDER BY LENGTH(City) ASC LIMIT 1;
SELECT City, LENGTH(City) AS Max_Length_City FROM Station ORDER BY LENGTH(City) DESC LIMIT 1;
...
insert into Product2 values(1,'S8',1000);
insert into Product2 values(2,'G4',800);
insert into Product2 values(3,'Pro',1400);
...
quantity int,
price int,
constraint fk Foreign key (product_id) References Product2(Product_id));
...
SELECT product_id, product_name FROM Product2 WHERE 
